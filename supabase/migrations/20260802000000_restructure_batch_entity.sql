-- MIGRATION: Restructure Batches and Entities (Make Batches top-level)

-- 1. Remove entity_id from batches
ALTER TABLE batches DROP COLUMN entity_id CASCADE;

-- 2. Remove entity_id from students
ALTER TABLE students DROP COLUMN entity_id CASCADE;

-- 3. Replace the get_dashboard_metrics RPC to handle the new structure
...
BEGIN
    ...
    FROM (
        SELECT 
            e.id, e.name,
            COALESCE((SELECT SUM(amount) FROM fee_components fc WHERE fc.entity_id = e.id), 0) as expected,
    ...
    -- 8. Batch Stats (Expected snapshot, Collected/Expenses in range)
